...
@dataclass
class AppEntry:
    """Represents a single app in a campaign."""
    app_name: str
    config_path: str
    app_path: str
    run_count: int = 3
    ...
    def from_dict(cls, data):
        """Create AppEntry from dictionary."""
        return cls(
            app_name=data.get("app_name", data.get("game_name", "Unknown App")), # Backward compatibility
            config_path=data.get("config_path", ""),
            app_path=data.get("app_path", data.get("game_path", "")), # Backward compatibility
...
class HybridConfigParser:
    ...
    def __init__(self, config_path: str):
        ...
        self.app_name = self.config.get("metadata", {}).get("app_name", self.config.get("metadata", {}).get("game_name", "Unknown App"))
        logging.getLogger(__name__).info(f"HybridConfigParser initialized for {self.app_name} using {config_path} (type: {self.config_type})")
    ...
    def get_app_metadata(self):
        """Get app metadata from the configuration."""
...
class SUTController:
    ...
    def __init__(self, name, ip, port, config_path="", app_path="", color=None):
        """Initialize SUT controller."""
        self.name = name
        self.ip = ip
        self.port = port
        self.config_path = config_path
        self.app_path = app_path # Renamed from game_path
        ...
        # Campaign mode (NEW)
        self.campaign_mode = False          # True if using campaign, False for single app
        self.campaign_name = "Default"      # Name of campaign
        self.campaign = []                  # List of AppEntry objects
        self.delay_between_apps = 120      # Seconds to wait between apps
        self.continue_on_failure = True     # Continue campaign if an app fails

        # Campaign progress tracking (NEW)
        self.total_apps = 0                # Total apps in campaign
        self.current_app_index = 0         # Which app (0-based)
        self.current_app_name = ""         # Name of current app
        self.failed_apps = []              # List of failed app names

        # App process tracking
        self.current_process_id = None      # Process ID of currently running app
    ...
    def start_automation(self, shared_settings):
        ...
        # Validation: Campaign mode vs Single app mode
        if self.campaign_mode:
            # Campaign mode: Check if campaign has apps
            if not self.campaign or len(self.campaign) == 0:
                self.status = "Error"
                return False
        else:
            # Single app mode: Check if config path is set
        ...
        if self.campaign_mode:
            # For campaign, total runs is sum of all app runs
            self.total_runs = sum(app.run_count for app in self.campaign)
        else:
            # For single app, use controller's run_count
    ...
    def _run_automation(self, shared_settings):
        """Main automation logic (runs in separate thread) - supports both single app and campaign modes."""
        try:
            ...
            # Check mode: Campaign or Single App
    ...
    def _run_single_app(self, shared_settings):
        """Run single app mode (original behavior)."""
        self.logger.info(f"Mode: Single App")
    ...
    def _run_campaign(self, shared_settings):
        ...
        # Loop through each app in campaign
        for app_index, app_entry in enumerate(self.campaign):
            # Check for stop event before each app
            if self.stop_event.is_set():
                self.logger.info(f"Campaign stopped by user before app {app_index + 1}")
                self.status = "Stopped"
                return

            # Update campaign progress
            self.current_app_index = app_index
            self.current_app_name = app_entry.app_name

            self.logger.info(f"")
            self.logger.info(f"╔════════════════════════════════════════════════════════════╗")
            self.logger.info(f"║ APP {app_index + 1}/{len(self.campaign)}: {app_entry.app_name}")
            self.logger.info(f"╚════════════════════════════════════════════════════════════╝")
            self.logger.info(f"Config: {app_entry.config_path}")
            self.logger.info(f"App Path: {app_entry.app_path}")
            self.logger.info(f"Runs: {app_entry.run_count}, Delay: {app_entry.run_delay}s")

            # Load config for this app
            try:
                config_parser = HybridConfigParser(app_entry.config_path)
                config = config_parser.get_config()
                config_type = config_parser.get_config_type()
                self.logger.info(f"Config type: {config_type}")
            except Exception as e:
                self.logger.error(f"Failed to load config for {app_entry.app_name}: {str(e)}")
                self.failed_apps.append(app_entry.app_name)

                if self.continue_on_failure:
                    self.logger.warning(f"⚠️  Skipping {app_entry.app_name} and continuing with campaign...")
                    continue  # Skip to next app
                else:
                    self.status = "Failed"
                    return  # Stop campaign

            # Create app folder within campaign
            app_folder_name = f"App-{app_index + 1}_{sanitize_folder_name(app_entry.app_name)}"
            app_dir = f"{campaign_dir}/{app_folder_name}"
            os.makedirs(app_dir, exist_ok=True)
            self.logger.info(f"App directory: {app_dir}")

            # Temporarily set app path for this app
            original_app_path = self.app_path
            self.app_path = app_entry.app_path

            # Run this app multiple times
            for run_num in range(1, app_entry.run_count + 1):
                # Check for stop event before each run
                if self.stop_event.is_set():
                    self.logger.info(f"Campaign stopped during App {app_index + 1}, Run {run_num}")
                    self.status = "Stopped"
                    self.app_path = original_app_path  # Restore
                    return

                self.current_run = run_num
                self.total_runs = app_entry.run_count

                self.logger.info(f"")
                self.logger.info(f"────────────────────────────────────────────────────────────")
                self.logger.info(f"  Run {run_num}/{app_entry.run_count} of {app_entry.app_name}")
                self.logger.info(f"────────────────────────────────────────────────────────────")

                # Run appropriate automation type
                if config_parser.is_step_based():
                    self._run_simple_automation(config_parser, config, shared_settings, app_dir, run_num)
                else:
                    self._run_state_machine_automation(config_parser, config, shared_settings, app_dir, run_num)
                ...
                if self.status == "Failed":
                    self.logger.error(f"❌ Run {run_num} of {app_entry.app_name} failed")
                    self.failed_apps.append(f"{app_entry.app_name} (Run {run_num})")

                    # Kill the failed app process before continuing
                    if self.current_process_id:
                        self.logger.info("Killing failed app process before continuing...")
                        self.kill_app_process()

                    if self.continue_on_failure:
                        self.logger.warning(f"⚠️  Skipping remaining runs of {app_entry.app_name} and continuing with campaign...")
                        self.status = "Running"  # Reset status to continue
                        break  # Skip remaining runs of this app, move to next app
                    else:
                        self.app_path = original_app_path  # Restore
                        return  # Stop campaign

                # Delay between runs (except after last run of this app)
                if run_num < app_entry.run_count and app_entry.run_delay > 0:
                    self.logger.info(f"Waiting {app_entry.run_delay} seconds before next run...")
                    for _ in range(app_entry.run_delay):
                        if self.stop_event.is_set():
            ...
            self.app_path = original_app_path

            self.logger.info(f">>>> Completed all {app_entry.run_count} runs of {app_entry.app_name}")

            # Delay between apps (except after last app)
            if app_index < len(self.campaign) - 1 and self.delay_between_apps > 0:
                self.logger.info(f"")
                self.logger.info(f"⏳ Waiting {self.delay_between_apps} seconds before next app...")
                for _ in range(self.delay_between_apps):
                    if self.stop_event.is_set():
                        self.logger.info("Campaign stopped during app delay")
                        self.status = "Stopped"
                        return
                    time.sleep(1)
        ...
        if self.status != "Failed" and self.status != "Stopped":
            ...
            if len(self.failed_apps) == 0:
                ...
                self.logger.info(f"║ Total Apps: {len(self.campaign)}")
                self.logger.info(f"║ All apps executed without errors")
            else:
                self.status = "Completed"  # Still mark as completed (partial success)
                self.logger.info(f"║ ⚠️  CAMPAIGN COMPLETED WITH FAILURES")
                self.logger.info(f"║ Total Apps: {len(self.campaign)}")
                self.logger.info(f"║ Successful: {len(self.campaign) - len(self.failed_apps)}")
                self.logger.info(f"║ Failed: {len(self.failed_apps)}")
                self.logger.info(f"║")
                self.logger.info(f"║ Failed Apps:")
                for failed_app in self.failed_apps:
                    self.logger.info(f"║   • {failed_app}")
    ...
    def _run_simple_automation(self, config_parser, config, shared_settings, batch_dir, run_num):
        ...
        try:
            ...
            from modules.app_launcher import AppLauncher # Renamed from game_launcher
            ...
            annotator = Annotator()
            app_launcher = AppLauncher(network) # Renamed from game_launcher
            ...
            # Get app metadata
            app_metadata = config_parser.get_app_metadata() # Renamed from game_metadata
            self.logger.info(f"App metadata loaded: {app_metadata}")
            startup_wait = app_metadata.get("startup_wait", 30)
            process_id = app_metadata.get("process_id", '')

            try:
                # Handle Steam Login FIRST - STOP if it fails
                steam_login_ok = _handle_steam_login(app_metadata)
                ...
                if self.app_path: # Renamed from game_path
                    self.logger.info(f"Launching app from: {self.app_path}")
                    # Pass process_id and startup_wait to enable process tracking on SUT
                    app_launcher.launch(self.app_path, process_id, startup_wait) # Renamed from game_launcher

                    # Store process ID for cleanup
                    if process_id:
                        self.current_process_id = process_id
                        self.logger.debug(f"Tracking app process: {process_id}")

                    # Wait for app to initialize
                    self.logger.info(f"Waiting {startup_wait} seconds for app to initialize...")
                    ...
                    self.logger.info("No app path provided, assuming app is already running")
                ...
                # Kill app process ONLY if failed or stopped (not on success)
                if self.current_process_id and self.status in ["Failed", "Stopped"]:
                    self.logger.info("Cleaning up: Killing app process...")
                    self.kill_app_process()
                elif self.current_process_id and self.status == "Completed":
                    self.logger.debug("Automation completed successfully - leaving app process running")
    ...
    def _run_state_machine_automation(self, config_parser, config, shared_settings, batch_dir, run_num):
        ...
        try:
            ...
            try:
                # Handle Steam Login FIRST - STOP if it fails
                steam_user = app_metadata.get("steam_username")
                steam_pass = app_metadata.get("steam_password")
    ...
    def to_dict(self):
        ...
        app_path_normalized = self.app_path.replace("\\", "/") if self.app_path else "" # Renamed from game_path
        ...
        return {
            ...
            "app_path": app_path_normalized, # Renamed from game_path
            "run_count": self.run_count,
            "run_delay": self.run_delay,
            # Campaign data (NEW)
            "campaign_mode": self.campaign_mode,
            "campaign_name": self.campaign_name,
            "campaign": [app.to_dict() for app in self.campaign], # Renamed from game
            "delay_between_apps": self.delay_between_apps, # Renamed from games
    ...
    def from_dict(data):
        ...
        controller.delay_between_apps = data.get("delay_between_apps", data.get("delay_between_games", 120)) # Backward compatibility
        controller.continue_on_failure = data.get("continue_on_failure", True)

        # Restore campaign apps
        campaign_data = data.get("campaign", [])
        controller.campaign = [AppEntry.from_dict(app_data) for app_data in campaign_data] # Renamed from game_data
        controller.total_apps = len(controller.campaign)

        return controller

    # Campaign management methods
    def add_app_to_campaign(self, app_entry): # Renamed from game
        """Add an app to the campaign."""
        self.campaign.append(app_entry)
        self.total_apps = len(self.campaign)
        self.campaign_mode = True  # Auto-enable campaign mode

    def remove_app_from_campaign(self, index): # Renamed from game
        """Remove an app from the campaign by index."""
    ...
    def move_app_up(self, index):
        """Move an app up in the campaign order."""
        if 0 < index < len(self.campaign):
    ...
    def move_app_down(self, index):
        """Move an app down in the campaign order."""
        if 0 <= index < len(self.campaign) - 1:
            self.campaign[index], self.campaign[index+1] = self.campaign[index+1], self.campaign[index]
            return True
        return False

    def clear_campaign(self):
        """Clear all apps from the campaign."""
        self.campaign.clear()
        self.total_apps = 0
        self.campaign_mode = False

    def kill_app_process(self, process_id=None):
        """Kill the app process on the SUT."""
...
def sanitize_folder_name(name):
    """Sanitize an app name for use in folder names."""
...
class MultiSUTGUI:
    ...
    def _create_sut_tab_content(self, name, tab_frame, controller):
        ...
        # Define app_var early so it's available for callbacks and preview updates
        app_var = tk.StringVar(value=controller.app_path)
        ...
        ttk.Radiobutton(mode_frame, text="Single App", variable=mode_var, value="single",
        ...
        # === SINGLE APP MODE FRAME ===
        single_app_frame = ttk.Frame(workflow_frame)

        # Row 1: Config dropdown selector
        sg_row1 = ttk.Frame(single_app_frame)
        ...
        def on_config_select(event=None):
            ...
            if selected_name and selected_name in config_dict:
                ...
                # Update preview and auto-load app path
                self._update_config_preview(controller, config_preview_frame, app_var)
        ...
        config_preview_frame = ttk.LabelFrame(single_app_frame, text="Workflow Details", padding="5")
        ...
        # App path row
        sg_row2 = ttk.Frame(single_app_frame)
        sg_row2.pack(fill=tk.X, pady=5)
        ttk.Label(sg_row2, text="App Path:").pack(side=tk.LEFT, padx=(0, 5))
        app_entry = ttk.Entry(sg_row2, textvariable=app_var, width=30)
        app_entry.pack(side=tk.LEFT, padx=(0, 5), fill=tk.X, expand=True)
        ttk.Button(sg_row2, text="Clear", command=lambda: app_var.set(""),
                  width=6).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Button(sg_row2, text="Verify", command=lambda: self._verify_app_path(controller),
                  width=6).pack(side=tk.LEFT)

        widgets['app_var'] = app_var

        # Run iteration settings
        sg_row3 = ttk.Frame(single_app_frame)
        ...
        widgets['single_app_frame'] = single_app_frame
        ...
        ttk.Label(header_row1, text="Delay between apps (sec):").pack(side=tk.LEFT, padx=(0, 5))
        delay_between_apps_var = tk.IntVar(value=controller.delay_between_apps)
        ttk.Spinbox(header_row1, from_=0, to=3600, textvariable=delay_between_apps_var,
                   width=8).pack(side=tk.LEFT)
        ...
        ttk.Checkbutton(header_row2, text="Continue campaign if an app fails (recommended for unattended runs)",
                       variable=continue_on_failure_var).pack(side=tk.LEFT)

        widgets['campaign_name_var'] = campaign_name_var
        widgets['delay_between_apps_var'] = delay_between_apps_var
        ...
        # App list section
        app_list_label = ttk.LabelFrame(campaign_frame, text="Apps in Campaign", padding="5")
        app_list_label.pack(fill=tk.BOTH, expand=True, pady=(0, 10))

        # Scrollable frame for app list
        canvas = tk.Canvas(app_list_label, height=200)
        scrollbar = ttk.Scrollbar(app_list_label, orient="vertical", command=canvas.yview)
        ...
        # Add app button
        add_app_btn_frame = ttk.Frame(campaign_frame)
        add_app_btn_frame.pack(fill=tk.X, pady=(0, 10))
        ttk.Button(add_app_btn_frame, text="+ Add App to Campaign",
                  command=lambda: self._add_app_dialog(controller, widgets)).pack(side=tk.LEFT, padx=(0, 10))

        # Campaign statistics
        campaign_stats_var = tk.StringVar(value=self._get_campaign_stats(controller))
        campaign_stats_label = ttk.Label(add_app_btn_frame, textvariable=campaign_stats_var,
        ...
        # Update preview if config already selected (for single app mode)
        if controller.config_path:
            self._update_config_preview(controller, config_preview_frame, app_var)
        ...
        if controller.campaign_mode:
            ...
            single_app_frame.pack(fill=tk.BOTH, expand=True)
    ...
    def _start_sut(self, controller):
        ...
        if controller.campaign_mode:
            ...
            controller.delay_between_apps = widgets.get('delay_between_apps_var').get()
            ...
            # Single app mode: update single app settings
            controller.app_path = widgets['app_var'].get()
        ...
        if controller.start_automation(shared_settings):
            ...
            if controller.campaign_mode:
                messagebox.showwarning("Warning", f"Cannot start {controller.name}: Campaign is empty. Add apps first.")
            else:
    ...
    def _verify_app_path(self, controller):
        """Verify app path on SUT."""
        messagebox.showinfo("Info", "App path verification not yet implemented")

    def _reload_config(self, controller, app_var):
        """Reload config file for SUT and auto-populate app path."""
        widgets = self.sut_widgets[controller.name]
        ...
        try:
            ...
            # Auto-load app path from config
            self._auto_load_app_path(controller, app_var)
            messagebox.showinfo("Success", f"Config loaded: {config_parser.app_name} ({config_parser.config_type})")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load config: {str(e)}")

    def _auto_load_app_path(self, controller, app_var):
        """Auto-populate app path from config file metadata."""
        ...
        try:
            ...
            metadata = config_parser.get_app_metadata()

            # Get app path from metadata
            app_path_in_config = metadata.get("path", "")

            if app_path_in_config:
                # Auto-populate app path
                app_var.set(app_path_in_config)
                controller.app_path = app_path_in_config

                # Log to SUT's log queue if logger exists
                if controller.logger:
                    controller.logger.info(f"Auto-populated app path from config: {app_path_in_config}")
            else:
    ...
    def _load_available_configs(self):
        """Scan config/apps folder and return dict of {app_name: file_path}."""
        config_dict = {}
        ...
        try:
            for filename in os.listdir(config_dir):
                if filename.endswith('.yaml') or filename.endswith('.yml'):
                    ...
                    try:
                        # Parse config to get app name
                        config_parser = HybridConfigParser(filepath)
                        app_name = config_parser.app_name

                        # Use app name as key, if duplicate add filename
                        display_name = app_name
                        if display_name in config_dict:
                            display_name = f"{app_name} ({filename})"
    ...
    def _update_config_preview(self, controller, preview_frame, game_var):
        ...
        try:
            ...
            # Build preview text
            preview_lines = []
            preview_lines.append(f"App Name:       {config_parser.app_name}")
            ...
            if metadata.get("path"):
                preview_lines.append(f"App Path:       {metadata.get('path')}")
    ...
    def _sync_controller_from_gui(self, controller):
        ...
        if 'delay_between_apps_var' in widgets:
            controller.delay_between_apps = widgets['delay_between_apps_var'].get()
        if 'continue_on_failure_var' in widgets:
            controller.continue_on_failure = widgets['continue_on_failure_var'].get()

        # Single App Settings
        if 'app_var' in widgets:
            controller.app_path = widgets['app_var'].get()
    ...
    def _get_campaign_stats(self, controller):
        ...
        total_apps = len(controller.campaign)
        total_runs = sum(app.run_count for app in controller.campaign)
        return f"Total: {total_apps} apps, {total_runs} runs"

    def _refresh_campaign_list(self, controller, widgets):
        """Refresh the campaign app list display."""
        ...
        if not controller.campaign:
            # Empty state
            ttk.Label(list_frame, text="No apps in campaign. Click '+ Add App to Campaign' to start.",
                     foreground="gray", font=("TkDefaultFont", 9)).pack(pady=20)
        else:
            # Display each app
            for idx, app in enumerate(controller.campaign):
                self._create_app_entry_widget(list_frame, controller, widgets, idx, app)
    ...
    def _create_app_entry_widget(self, parent, controller, widgets, index, app):
        """Create a single app entry widget in the campaign list."""
        # App entry frame with border
        app_frame = ttk.Frame(parent, relief="solid", borderwidth=1)
        app_frame.pack(fill=tk.X, pady=2, padx=2)

        # Left section: App number and info
        left_section = ttk.Frame(app_frame)
        left_section.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5, pady=5)

        # Row 1: Number and app name
        ...
        ttk.Label(row1, text=app.app_name, font=("TkDefaultFont", 10, "bold")).pack(side=tk.LEFT)

        # Row 2: Config file
        row2 = ttk.Frame(left_section)
        row2.pack(fill=tk.X)
        ttk.Label(row2, text=f"Config: {os.path.basename(app.config_path)}",
                 font=("TkDefaultFont", 8), foreground="gray").pack(side=tk.LEFT)

        # Row 3: Run settings
        row3 = ttk.Frame(left_section)
        row3.pack(fill=tk.X)
        ttk.Label(row3, text=f"Runs: {app.run_count} | Delay: {app.run_delay}s",
                 font=("TkDefaultFont", 8), foreground="gray").pack(side=tk.LEFT)

        # Right section: Action buttons
        right_section = ttk.Frame(app_frame)
        right_section.pack(side=tk.RIGHT, padx=5, pady=5)

        button_size = 6
        ttk.Button(right_section, text="Edit", width=button_size,
                  command=lambda: self._edit_app_dialog(controller, widgets, index)).pack(side=tk.LEFT, padx=2)
        ttk.Button(right_section, text="↑", width=3,
                  command=lambda: self._move_app_up(controller, widgets, index)).pack(side=tk.LEFT, padx=2)
        ttk.Button(right_section, text="↓", width=3,
                  command=lambda: self._move_app_down(controller, widgets, index)).pack(side=tk.LEFT, padx=2)
        ttk.Button(right_section, text="Remove", width=button_size,
                  command=lambda: self._remove_app_from_campaign(controller, widgets, index)).pack(side=tk.LEFT, padx=2)

    def _add_app_dialog(self, controller, widgets):
        """Show dialog to add a new app to the campaign."""
        dialog = tk.Toplevel(self.root)
        dialog.title("Add App to Campaign")
        ...
        # App name (auto-filled from config)
        ttk.Label(content_frame, text="App Name:").grid(row=2, column=0, sticky=tk.W, pady=5)
        app_name_var = tk.StringVar()
        ttk.Entry(content_frame, textvariable=app_name_var, width=40).grid(row=2, column=1, pady=5, padx=(10, 0))

        # App path (auto-filled from config)
        ttk.Label(content_frame, text="App Path:").grid(row=3, column=0, sticky=tk.W, pady=5)
        app_path_var = tk.StringVar()
        ttk.Entry(content_frame, textvariable=app_path_var, width=40).grid(row=3, column=1, pady=5, padx=(10, 0))
        ...
        def on_config_selected(event=None):
            ...
            if selected_name and selected_name in config_dict:
                ...
                try:
                    config_parser = HybridConfigParser(config_path)
                    metadata = config_parser.get_app_metadata()
                    app_name_var.set(config_parser.app_name)
                    app_path_var.set(metadata.get("path", ""))
                except Exception as e:
                    app_name_var.set(selected_name)
        ...
        def on_add():
            ...
            app_entry = AppEntry(
                ...
                app_path=app_path_var.get(),
                run_count=run_count_var.get(),
                run_delay=run_delay_var.get()
            )

            # Add to campaign
            controller.add_app_to_campaign(app_entry)
    ...
    def _edit_app_dialog(self, controller, widgets, index):
        """Show dialog to edit an app in the campaign."""
        if index < 0 or index >= len(controller.campaign):
            return

        app = controller.campaign[index]

        dialog = tk.Toplevel(self.root)
        dialog.title(f"Edit App: {app.app_name}")
        ...
        ttk.Label(content_frame, text=os.path.basename(app.config_path), foreground="blue").grid(row=0, column=1, sticky=tk.W, pady=5, padx=(10, 0))

        # App name
        ttk.Label(content_frame, text="App Name:").grid(row=1, column=0, sticky=tk.W, pady=5)
        app_name_var = tk.StringVar(value=app.app_name)
        ttk.Entry(content_frame, textvariable=app_name_var, width=40).grid(row=1, column=1, pady=5, padx=(10, 0))

        # App path
        ttk.Label(content_frame, text="App Path:").grid(row=2, column=0, sticky=tk.W, pady=5)
        app_path_var = tk.StringVar(value=app.app_path)
        ttk.Entry(content_frame, textvariable=app_path_var, width=40).grid(row=2, column=1, pady=5, padx=(10, 0))

        # Run count
        ttk.Label(content_frame, text="Run Count:").grid(row=3, column=0, sticky=tk.W, pady=5)
        run_count_var = tk.IntVar(value=app.run_count)
        ...
        run_delay_var = tk.IntVar(value=app.run_delay)
        ...
        def on_save():
            # Update app entry
            app.app_name = app_name_var.get()
            app.app_path = app_path_var.get()
            app.run_count = run_count_var.get()
            app.run_delay = run_delay_var.get()
    ...
    def _move_app_up(self, controller, widgets, index):
        """Move an app up in the campaign order."""
        controller.move_app_up(index)
        self._refresh_campaign_list(controller, widgets)

    def _move_app_down(self, controller, widgets, index):
        """Move an app down in the campaign order."""
        controller.move_app_down(index)
        self._refresh_campaign_list(controller, widgets)

    def _remove_app_from_campaign(self, controller, widgets, index):
        """Remove an app from the campaign."""
        if index < 0 or index >= len(controller.campaign):
            return

        app = controller.campaign[index]
        if messagebox.askyesno("Confirm Removal", f"Remove '{app.app_name}' from campaign?"):
            controller.remove_app_from_campaign(index)
            self._refresh_campaign_list(controller, widgets)
    ...
    def _clear_campaign(self, controller, widgets):
        ...
        if messagebox.askyesno("Confirm Clear", f"Remove all {len(controller.campaign)} apps from campaign?"):
    ...
    def _save_campaign(self, controller):
        ...
        if not controller.campaign:
            messagebox.showwarning("Warning", "Campaign is empty. Add apps before saving.")
            return
        ...
        campaign_data = {
            ...
            "delay_between_apps": controller.delay_between_apps,
            "continue_on_failure": controller.continue_on_failure,
            "apps": [app.to_dict() for app in controller.campaign]
        }
    ...
    def _load_campaign(self, controller, widgets):
        ...
        try:
            ...
            controller.delay_between_apps = campaign_data.get("delay_between_apps", campaign_data.get("delay_between_games", 120))
            controller.continue_on_failure = campaign_data.get("continue_on_failure", True)

            # Load apps
            controller.campaign.clear()
            for app_data in campaign_data.get("apps", campaign_data.get("games", [])):
                app_entry = AppEntry.from_dict(app_data)
                controller.campaign.append(app_entry)
            ...
            if widgets:
                ...
                widgets.get('delay_between_apps_var').set(controller.delay_between_apps)
            ...
            messagebox.showinfo("Success", f"Loaded {len(controller.campaign)} apps from {os.path.basename(filename)}")
    ...
    def update_gui(self):
        ...
        for name, controller in self.sut_controllers.items():
            ...
            if controller.campaign_mode and controller.status == "Running":
                # Campaign mode: show app and run progress
                app_progress = f"App: {controller.current_app_index + 1}/{controller.total_apps}"
                run_progress = f"Run: {controller.current_run}/{controller.total_runs}"
                app_name_short = controller.current_app_name[:20] + "..." if len(controller.current_app_name) > 20 else controller.current_app_name
                run_label.config(text=f"{app_progress} ({app_name_short}) | {run_progress}")
            else:
                # Single app mode: show run progress only
