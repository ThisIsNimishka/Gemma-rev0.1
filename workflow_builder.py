"""
Interactive Workflow Builder GUI for App Automation
Build, test, and export app automation workflows visually

Features:
- App control and screenshot capture
...
class WorkflowBuilderGUI:
    ...
    def __init__(self, root):
        ...
        # SUT connection
        self.sut_ip = tk.StringVar(value="192.168.50.230") #PTL
        ...
        # Metadata variables
        self.app_name = tk.StringVar(value="My App")
        self.app_path = tk.StringVar(value="")
    ...
    def create_widgets(self):
        ...
        # Compact metadata fields with Test/Kill button for App Path
        row = 0
        
        # Regular fields before App Path
        ...
        # App Path with Test/Kill button
        ttk.Label(meta_scrollable, text="App Path:").grid(row=row, column=0, sticky=tk.W, pady=1, padx=2)
        app_path_frame = ttk.Frame(meta_scrollable)
        app_path_frame.grid(row=row, column=1, columnspan=2, sticky=tk.W, pady=1)
        ttk.Entry(app_path_frame, textvariable=self.app_path, width=20).pack(side=tk.LEFT)
        
        # Test/Kill button (starts as "▶ Test")
        self.app_running = False
        self.test_kill_btn = tk.Button(
            app_path_frame, text="▶ Test", width=6, 
            command=self.toggle_app_test,
    ...
    def toggle_app_test(self):
        """Toggle between launching app and killing it."""
        if not self.network:
        ...
        if not self.app_running:
            ...
            if not app_path:
                messagebox.showerror("Error", "Enter an App Path first!")
                return
            
            try:
                self.status_text.set("Launching app...")
                self.root.update()
                
                # Launch app via SUT
                response = self.network.session.post(
                    f"{self.network.base_url}/launch",
                    json={"path": app_path, "process_id": self.process_id.get()},
                    timeout=90
                )
                ...
                if result.get("status") == "success":
                    ...
                    process_name = self.process_id.get() or os.path.basename(app_path)
                    self.status_text.set(f"App launched! Process: {process_name}")
                else:
            ...
            # KILL mode - use existing /action with terminate_app
            try:
                self.status_text.set("Killing app...")
                self.root.update()
                
                # Kill via existing action endpoint
                result = self.network.send_action({"type": "terminate_app"})
                
                if result.get("status") == "success":
                    self.status_text.set("App terminated")
                else:
    ...
    def _check_app_process(self):
        ...
        if not self.network or not self.app_running:
            return
        ...
        try:
            ...
            if result.get("running"):
                ...
                self.status_text.set("App exited")
        except Exception as e:
    ...
    def _execute_sideload(self, step):
        """Execute sideload script for a step."""
        if not self.network or not step.sideload_config:
            return

        config = step.sideload_config
        path = config.get("path")
        if not path:
            return

        logger.info(f"Executing sideload for step {step.step_number}: {path}")
        try:
            ...
            if result.get("status") == "success":
                self.status_text.set(f"Step {step.step_number}: Sideload completed successfully")
            else:
                logger.warning(f"Sideload failed: {result.get('error', 'Unknown error')}")
                self.status_text.set(f"Step {step.step_number}: Sideload failed")
        except Exception as e:
            logger.error(f"Sideload execution error: {e}")
            self.status_text.set(f"Step {step.step_number}: Sideload error")

    def _execute_pre_hooks(self):
        """Execute pre-automation hooks."""
        if not self.network or not self.hooks.get("pre"):
            return

        self.status_text.set("Executing pre-hooks...")
        self.root.update()

        for hook in self.hooks["pre"]:
            if self.flow_stop_requested: break
            
            path = hook.get("path")
            if not path: continue

            is_persistent = hook.get("persistent", False)
            logger.info(f"Executing {'persistent ' if is_persistent else ''}pre-hook: {path}")
            ...
            try:
                result = self.network.execute_command(
                    ...
                    wait=not is_persistent
                )
                
                if is_persistent and result.get("status") == "success":
                    pid = result.get("pid")
                    if pid:
                        self.persistent_processes[pid] = path
                        logger.info(f"Persistent hook started with PID: {pid}")
            except Exception as e:
                logger.error(f"Pre-hook execution failed: {e}")

    def _execute_post_hooks(self):
        """Execute post-automation hooks."""
        if not self.network or not self.hooks.get("post"):
            return

        self.status_text.set("Executing post-hooks...")
        self.root.update()
        ...
        for hook in self.hooks["post"]:
            ...
            logger.info(f"Executing post-hook: {path}")
            try:
    ...
    def _stop_persistent_hooks(self):
        ...
        if not self.network or not self.persistent_processes:
            return

        self.status_text.set("Stopping persistent hooks...")
        self.root.update()

        for pid, path in list(self.persistent_processes.items()):
            logger.info(f"Stopping persistent hook: {path} (PID: {pid})")
            try:
                self.network.terminate_process(pid)
            except Exception as e:
                logger.warning(f"Failed to stop hook PID {pid}: {e}")
        
        self.persistent_processes.clear()

    def _update_hooks_toggle_label(self, *args):
        """Update checkbox label and styling based on state."""
        if not hasattr(self, 'hooks_toggle'):
            return
    ...
    def test_action(self):
        ...
        try:
            ...
            # Execute sideload if enabled and configured
            if self.enable_hooks_testing.get() and hasattr(step, 'sideload_config') and step.sideload_config:
                self.status_text.set(f"Step {step.step_number}: Executing sideload {step.sideload_config.get('path')}...")
                self.root.update()
                self._execute_sideload(step)
    ...
    def test_full_flow(self):
        ...
        # Check for hooks/sideload
        run_hooks = False
        if self.enable_hooks_testing.get() and (self.hooks["pre"] or self.hooks["post"]):
            run_hooks = messagebox.askyesno("Hooks", "Execute pre/post automation hooks for this flow?")
        ...
        # Execute pre-hooks if requested
        if run_hooks:
            self._execute_pre_hooks()
            if self.flow_stop_requested: 
                self.flow_running = False
                self.flow_btn.config(text="▶▶ Flow", bg="#2E7D32")
                return
        ...
        for i in range(start_idx, len(self.workflow_steps)):
            ...
            try:
                ...
                # Execute sideload if enabled and configured (after main action)
                if self.enable_hooks_testing.get() and hasattr(step, 'sideload_config') and step.sideload_config:
                    self.status_text.set(f"Step {i + 1}: Executing sideload...")
                    self.root.update()
                    self._execute_sideload(step)
        ...
        # Cleanup hooks
        if run_hooks or self.persistent_processes:
    ...
    def save_yaml(self):
        ...
        try:
            ...
            if self.app_path.get():
                metadata["path"] = self.app_path.get()
    ...
    def load_yaml(self):
        ...
        try:
            ...
            # Compatibility: check for app_name first, then fallback to game_name
            name = metadata.get("app_name", metadata.get("game_name", "Loaded App"))
            self.app_name.set(name)
            self.version.set(metadata.get("version", "1.0"))
            self.app_path.set(metadata.get("path", ""))
...
"contains"    RECOMMENDED - Most flexible!
              Finds "PLAY" in "PLAY APP" or "CLICK TO PLAY"
            - Exact: Must match perfectly (ignoring case)
              "Play" won't match "PLAY" or "Play App"
            - Starts With: Must start with the text
              "Play" matches "Play App" but not "Click Play"
            - Ends With: Must end with the text
              "App" matches "Play App" but not "App Error"
...
Example uses:
  ...
  - Run a setup script before the app starts
...
Solutions:
  1. Use more of the text (e.g., "Play App" instead of "Play")
