...
def resolve_steam_app_path(app_id, target_process_name=''):
    ...
    full_app_path = os.path.join(app_library, "steamapps", "common", install_dir_name)
    logger.info(f"App folder resolved: {full_app_path}")
    
    if not os.path.exists(full_app_path):
         return None, f"App folder does not exist: {full_app_path}"
    ...
    if target_process_name:
        exe_path = os.path.join(full_app_path, f"{target_process_name}.exe")
        if os.path.exists(exe_path):
            return exe_path, None
        
        # Search recursively for target process name
        for root, dirs, files in os.walk(full_app_path):
    ...
    exe_path = os.path.join(full_app_path, f"{install_dir_name}.exe")
    if os.path.exists(exe_path):
        return exe_path, None
    ...
    return None, "No executable found in app directory"
...
def launch_app():
    """Launch an app with process tracking. Supports both exe paths and Steam app IDs."""
    global app_process, current_app_process_name
    ...
    try:
        ...
        # Validate app_path is provided (before conversion)
        if not app_path:
            logger.error("No app path provided")
            return jsonify({"status": "error", "error": "App path is required"}), 400

        # Convert app_path to string
        ...
        if app_path.isdigit():
            is_steam_id = True
            logger.info(f"Detected Steam App ID: {app_path}")
        elif app_path.startswith('steam://'):
            # Legacy handling, try to extract ID
            match = re.search(r'run/(\d+)', app_path) or re.search(r'rungameid/(\d+)', app_path)
            if match:
                app_path = match.group(1)
                is_steam_id = True
                logger.info(f"Extracted Steam App ID from URL: {app_path}")

        # Resolve Steam App ID to Executable
        steam_app_id = None  # Track the original app ID for steam:// launch
        if is_steam_id:
            steam_app_id = app_path  # Store original Steam App ID
            logger.info(f"Resolving Steam App ID: {app_path}")
            resolved_path, error = resolve_steam_app_path(app_path, process_id)
            if resolved_path:
                logger.info(f"Resolved Steam ID {app_path} to: {resolved_path}")
             ...
             if not os.path.exists(app_path):
                 ...
                 return jsonify({"status": "error", "error": "App executable not found"}), 404
             # For direct EXE, use process_id if provided, otherwise extract from path
             new_process_name = process_id if process_id else os.path.splitext(os.path.basename(app_path))[0]
        
        with app_lock:
            # Terminate existing app if running (using GLOBAL current_app_process_name)
            if current_app_process_name:
                logger.info(f"Terminating existing app: {current_app_process_name}")
                terminate_process_by_name(current_app_process_name)
                # Also try to terminate previous app_process handle
            ...
            current_app_process_name = new_process_name

            # Launch app - Use steam:// protocol for Steam apps to ensure Steam starts
            if steam_app_id:
                # Launch via Steam protocol - this automatically starts Steam if not running
                steam_url = f"steam://rungameid/{steam_app_id}"
                logger.info(f"Launching app via Steam protocol: {steam_url}")
                os.startfile(steam_url)
                app_process = None  # No subprocess handle for steam:// launch
            else:
                # Direct exe launch for non-Steam apps
                logger.info(f"Launching app directly: {app_path}")
                app_dir = os.path.dirname(app_path)
                try:
                    app_process = subprocess.Popen(app_path, cwd=app_dir)
                except Exception as e:
            ...
            # Log launch status
            if app_process:
                logger.info(f"Subprocess started with PID: {app_process.pid}")
            else:
                logger.info("App launched via Steam protocol (no direct PID)")

            time.sleep(3) # Initial wait for process spawn

            if app_process:
                subprocess_status = "running" if app_process.poll() is None else "exited"
                logger.info(f"Subprocess status after 3 seconds: {subprocess_status}")
            
            # If subprocess exited quickly (launcher wrapper), we need to find the actual app process
            # Give the actual app process time to start and show window
            max_wait_time = 60  # Increased wait time to ensure window logic has time
            wait_interval = 1
            actual_process = None
            foreground_confirmed = False
            
            logger.info(f"Waiting up to {max_wait_time}s for process '{current_app_process_name}' to appear...")

            # Phase 1: Detect Process
            start_wait = time.time()
            while time.time() - start_wait < max_wait_time:
                actual_process = find_process_by_name(current_app_process_name)
                if actual_process:
            ...
            if actual_process:
                ...
                response_data = {
                    ...
                    "subprocess_pid": app_process.pid if app_process else None,
                    "subprocess_status": subprocess_status if app_process else "steam_protocol",
                    "resolved_path": app_path if is_steam_id else None,
                    "launch_method": "steam" if is_steam_id else "direct_exe",
                    "app_process_pid": actual_process.pid,
                    "app_process_name": actual_process.name(),
                    "app_process_status": actual_process.status(),
                ...
                logger.warning(f"App process '{current_app_process_name}' not found within {max_wait_time} seconds")
                response_data = {
                    "status": "warning",
                    "warning": f"App process '{current_app_process_name}' not detected, but launch command executed",
                    "subprocess_pid": app_process.pid if app_process else None,
                    "subprocess_status": subprocess_status if app_process else "steam_protocol",
                    "resolved_path": app_path if is_steam_id else None,
                    "launch_method": "steam" if is_steam_id else "direct_exe"
    ...
    try:
        ...
        if action_type == 'click':
            ...
            with app_lock:
                ...
                if current_app_process_name:
                    if terminate_process_by_name(current_app_process_name):
                        terminated = True
                ...
                message = "App terminated" if terminated else "No app running"
                return jsonify({
                    "status": "success",
...
def health_check():
    ...
    try:
        ...
        if current_app_process_name:
            app_proc = find_process_by_name(current_app_process_name)
            health_status["app_process"] = "running" if app_proc else "not_found"
        else:
            health_status["app_process"] = "none"
