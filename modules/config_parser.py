"""
Simple Configuration Parser for App Automation
...
class AppConfigParser:
    ...
    def _validate_config(self) -> bool:
        ...
        for step_num, step in steps.items():
            ...
            # 3. Legacy format: has 'find_and_click' section
            has_modular_format = "find" in step and "action" in step
            has_wait_action = "action" in step and self._is_wait_action(step["action"])
            has_legacy_format = "find_and_click" in step
            
            if not (has_modular_format or has_wait_action or has_legacy_format):
                ...
                logger.error(f"  3. 'find_and_click' section (legacy format)")
            ...
            elif has_wait_action:
                self._validate_wait_action(step["action"], step_num)

            # Validate legacy format
            elif has_legacy_format:
                self._validate_find_section(step["find_and_click"], step_num)
