...
class SimpleAutomation:
    ...
    def __init__(self, config_path, network, screenshot_mgr, vision_model, stop_event=None, run_dir=None, annotator=None, progress_callback=None):
        ...
        # Load configuration
        try:
            from modules.simple_config_parser import AppConfigParser
            config_parser = AppConfigParser(config_path)
            self.config = config_parser.get_config()
            logger.info("Using AppConfigParser for step-based configuration")
        except (ImportError, ValueError):
            logger.info("AppConfigParser not available, loading YAML directly")
    ...
    def _handle_text_action(self, action_config: Dict[str, Any]) -> bool:
        ...
        # Type text using bulk action for efficiency and case sensitivity
        char_delay = action_config.get("char_delay", 0.05)
        
        try:
            # Send the entire text string as one action
            # This allows the SUT service to use pyautogui.write() which handles:
            # 1. Capital letters (Shift automatically handled)
            # 2. Special characters
            # 3. Much better performance (single network request)
            action = {
                "type": "text",
                "text": text,
