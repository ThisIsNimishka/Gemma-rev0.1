...
Supports flexible text matching and state context for any app benchmark.
"""
...
class DecisionEngine:
    """
    FSM-based decision engine with enhanced flexibility for different apps.
    """
    
    def __init__(self, config: Dict[str, Any]):
        """Initialize the decision engine with app-specific configuration."""
        self.config = config
        ...
        self.app_name = config.get("metadata", {}).get("app_name", "Unknown App")
        ...
        logger.info(f"DecisionEngine initialized for {self.app_name} with {len(self.states)} states")
