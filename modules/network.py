...
class NetworkManager:
    ...
    def launch_app(self, app_path: str, process_id: str = '', startup_wait: int = 15) -> Dict[str, Any]:
        """
        Request the SUT to launch an app.
        ...
        """
        try:
            # Prepare request payload with app metadata
            payload = {
                "path": app_path,
            ...
            response.raise_for_status()
            result = response.json()
            logger.info(f"App launch request sent: {app_path}, process_id: {process_id}, startup_wait: {startup_wait}")
            return result
        except requests.RequestException as e:
            logger.error(f"Failed to launch app {app_path}: {str(e)}")
            raise
