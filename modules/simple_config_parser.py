...
class AppConfigParser:
    ...
    def __init__(self, config_path: str):
        """
        Initialize the app config parser.
        ...
        self.app_name = self.config.get("metadata", {}).get("app_name", 
                        self.config.get("metadata", {}).get("game_name", "Unknown App"))
        logger.info(f"AppConfigParser initialized for {self.app_name} using {config_path}")
    ...
    def _validate_config(self) -> bool:
        ...
        for step_num, step in steps.items():
            ...
            # 4. A 'find_and_click' section (legacy format)
            has_find = "find" in step
            has_action = "action" in step
            has_sideload = "sideload" in step
            has_legacy_fac = "find_and_click" in step

            if not has_find and not has_action and not has_sideload and not has_legacy_fac:
                logger.error(f"Step {step_num} must have 'find', 'action', 'sideload' or 'find_and_click' section")
                raise ValueError(f"Invalid step {step_num}: missing required sections")

            # Validate legacy find_and_click
            if has_legacy_fac:
                fac_section = step["find_and_click"]
                if not isinstance(fac_section, dict):
                    logger.error(f"Step {step_num} 'find_and_click' must be a dictionary")
                    raise ValueError(f"Invalid step {step_num}: 'find_and_click' must be a dictionary")
    ...
    def get_metadata(self) -> Dict[str, Any]:
        ...
        Returns:
            Metadata dictionary with app information
        """
